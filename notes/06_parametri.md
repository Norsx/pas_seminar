...
| `xy_goal_tolerance` / `yaw_goal_tolerance` | **0.05 m / 0.05 rad** (bilo 0.10 / 0.05, prije toga 0.20 / 0.25) — **postoji na DVA mjesta i moraju biti jednaka**: `general_goal_checker` i `FollowPath` | `NAV` | **mora biti uže od gatea** (`centreline_tolerance` 0.08): na 0.10 je Nav2 smio parkirati 10 cm od portala, a gate odbija sve preko 8 cm ([[P-39_nav2_enters_doorway_at_an_angle]] #14) |
...
| Upravljač baze | **`DWBLocalPlanner`** (direktno), **20 Hz** (bilo 10 Hz) | `NAV` `FollowPath` | Uklonjen `RotationShimController` koji je ometao holonomsko gibanje omni baze; `Twirling.scale: 8.0` sprječava lutanje kuta u hodu; `debug_trajectory_details: False` uklanja serijalizaciju 15.625 trajektorija |
...
| `ObstacleFootprint.scale` / `PathDist.scale` / `GoalDist.scale` / `Twirling.scale` | **2 / 64 / 24 / 8** | `NAV` `FollowPath` | `PathAlign` uklonjen za omni bazu; `RotateToGoal.scale: 64.0` |
...
| DWB `max_vel_x` / `max_vel_y` / `max_vel_theta` | **0.30 / 0.30 / 0.60** | `NAV` `FollowPath` | omni raspon s 25 uzoraka po osi |
...
| zone: `chute_length` / `chute_thickness` | **0.85 / 0.65 m** | `nav_zones.py` `default_params` | vrijednost iz [[P-39_nav2_enters_doorway_at_an_angle]] #7 (1.20 → 0.85); 0.50 je bila iz povučenog sloja |
| zone: `lane_margin` | **−0.05 m** (negativno = šire) | `nav_zones.py` | traka je 1.05 m (vrata 0.95), dakle guide walls su **samo 5 cm po strani šire od otvora** → robotu ostaje 10.3 cm po strani unutar trake |
