...
| `xy_goal_tolerance` / `yaw_goal_tolerance` | **0.05 m / 0.05 rad** (bilo 0.10 / 0.05, prije toga 0.20 / 0.25) — **postoji na DVA mjesta i moraju biti jednaka**: `general_goal_checker` i `FollowPath` (čita ga `RotateToGoalCritic`) | `NAV` | **mora biti uže od gatea** (`centreline_tolerance` 0.08): na 0.10 je Nav2 smio parkirati 10 cm od portala, a gate odbija sve preko 8 cm — legalan dolazak je bio nelegalno stanje bez izlaza ([[P-39_nav2_enters_doorway_at_an_angle]] #14). Fizički budžet je 7.75 cm po strani, pa se gate ne može popustiti |
...
| **`footprint_publisher`** (novo 14. 9.) | 5 Hz, okvir `base_link`, `collision` geometrija, **`max_vertices` 8**, `change_tolerance` 0.01 m, `resend_period` 2.0 s, `max_reach` 1.50 m | `footprint_publisher.py`, diže ga `sim.launch.py` | objavljuje **stvarni** obris robota na `footprint` oba costmapa (latched + periodično, da ga uhvati i costmap koji se pretplati kasnije); statični `footprint` u `NAV` ostaje rezervna vrijednost ([[D-19_dynamic_footprint]]). **8 vrhova**: uključuju ±x i ±y pa su širina i duljina točne na 0.000 mm, a 24 vrha su upravljaču rušila 10 Hz petlju |
...
| `scan_filter` | `half_length` **0.45**, `half_width` **0.30** (bilo 0.75 / 0.47) | `scan_filter.py` | Dimenzionirano prema onome **što je na visini lasera** (0.2086 m): ploča u koju je laser ugrađen je 0.58 × 0.39 m, dakle 0.195 m polu-širine; veća kutija 0.717 × 0.497 počinje na z=0.2232, a ruke su na z ≥ 0.40. Prijašnjih 0.47 je pri bočnom otklonu > 3 cm brisalo dovratnik (na ±0.50) i zamrzavalo stare oznake u prolazu → 30 s stajanja pa recovery ([[P-39_nav2_enters_doorway_at_an_angle]] #13). Uže od 0.35 znači da su i noge stolova sada vidljive |
