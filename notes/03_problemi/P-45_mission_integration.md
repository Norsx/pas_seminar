...
| 20 | 16. 9. / svjež klon | GUI test svježeg klona s GitHuba: misija prošla sve faze do odlaganja, ali se kutija u koraku 8 nije odspojila od zapešća prije odmicanja ruku | ❌ kutija ostala visjeti na lijevom zapešću u `DETECTION_V4` i pri odmicanju robota | Uzrok: `_attach_box` je slao detach kroz efemerni `ign topic` subprocess bez provjere povratnog koda i bez ROS bridgea; UDP poruka je izgubljena u transportu, pa Gazebo nikada nije primio detach zahtjev (`DetachableJoint` ostao attached) |
| 21 | 16. 9. / radno stablo | `_attach_box` prebačen na trajni ROS 2 most (`/aruco_box/detach`, `std_msgs/Empty`), uz ign fallback u petlji i potvrdu stanja preko `/aruco_box/state` (`std_msgs/String`). Provjera povratne vrijednosti u `_place_on_marker` sprječava odmicanje ako se kutija nije odspojila | build prolazi (17/17) | Riješeno |
